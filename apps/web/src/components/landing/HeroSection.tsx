...
const trustLogos = [
  { name: "Vercel", color: "bg-foreground/10" },
  { name: "Stripe", color: "bg-foreground/10" },
  { name: "Supabase", color: "bg-foreground/10" },
  { name: "Auth0", color: "bg-foreground/10" },
  { name: "Neon", color: "bg-foreground/10" },
]
...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden pt-24 pb-16 sm:pt-32 sm:pb-24">
      <div className="absolute inset-0 bg-gradient-to-b from-muted/50 via-transparent to-background pointer-events-none" />
      <div className="absolute top-1/4 right-0 w-[600px] h-[600px] bg-muted/80 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-1/4 left-0 w-[400px] h-[400px] bg-muted/60 rounded-full blur-[100px] pointer-events-none" />
      ...
      >
        ...
        <div className={`mt-16 ${isVisible ? "animate-fade-in" : "opacity-0"}`} style={{ animationDelay: "300ms" }}>
          <div className="relative mx-auto max-w-5xl">
            <div className="relative overflow-hidden rounded-xl border bg-card/50 backdrop-blur-sm shadow-2xl">
              ...
              <div className="p-6">
                ...
                <div className="mt-3 flex items-center gap-3 rounded-lg border bg-card p-3 text-sm">
                  ...
                  <span className="font-semibold text-primary">aB3xK9m</span>
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-8 opacity-60">
              ...
              {trustLogos.map((logo) => (
                <span key={logo.name} className="flex items-center gap-2 text-sm font-semibold text-muted-foreground/40">
                  <span className={`inline-block h-3 w-3 rounded ${logo.color}`} />
                  {logo.name}
                </span>
              ))}
            </div>
