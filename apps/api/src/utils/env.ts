...
const envSchema = z.object({
  ...
  LINK_ACCESS_SECRET: z.string().length(64, 'LINK_ACCESS_SECRET must be exactly 64 hex characters'),
  ENCRYPTION_KEY: z.string().length(64, 'ENCRYPTION_KEY must be exactly 64 hex characters'),
  FINGERPRINT_SECRET: z.string().length(64, 'FINGERPRINT_SECRET must be exactly 64 hex characters'),
  CSRF_SECRET: z.string().length(64, 'CSRF_SECRET must be exactly 64 hex characters'),
