...
const envSchema = z.object({
  ...
  LINK_ACCESS_SECRET: z.string().length(64).regex(/^[0-9a-f]{64}$/, 'LINK_ACCESS_SECRET must be 64 hex characters'),
  ENCRYPTION_KEY: z.string().length(64).regex(/^[0-9a-f]{64}$/, 'ENCRYPTION_KEY must be 64 hex characters'),
  FINGERPRINT_SECRET: z.string().length(64).regex(/^[0-9a-f]{64}$/, 'FINGERPRINT_SECRET must be 64 hex characters'),
  CSRF_SECRET: z.string().length(64).regex(/^[0-9a-f]{64}$/, 'CSRF_SECRET must be 64 hex characters'),
